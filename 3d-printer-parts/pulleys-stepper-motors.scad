...
module stepper_nema_17(with_10mm=false) {
  ...
  ccylinder(d = 5, h = 63);
  black() {
    Tz(15) make_rounded_box([40, 40, 10]);
    make_rounded_box([37, 37, 20]);
    Tz(-15) make_rounded_box([40, 40, 10]);
  }
}
...
module stepper_nema_23(with_sprocket=false, for_cutout=false) {
  ...
  if (with_sprocket) {
    Tz(total_H/2 - 23.2/2 + 3) silver() pulley_gt2_10mm_sprocket_20();
  }
  ...
  black() D() {
    U() {
      // plate 56.3 56.3 5.6 R edge=3
       Tz(total_H/2 - shaft_H -1.6 - body_H /2 ) make_rounded_box([56.3, 56.3, body_H], d=6);
       if (for_cutout) {
          // bolts 9.9d 47 square
          Tz(total_H/2 - shaft_H -1.6 - body_H /2 )
          make_rounded_box_shafts(size=[47,47,body_H + 30], d=5.5, inset=0);
       }
    }
    U() {
       if (!for_cutout) {
        // bolts 9.9d 47 square
        Tz(total_H/2 - shaft_H -1.6 - body_H /2 ) make_rounded_box_shafts(size=[47,47,body_H + 3], d=4.9, inset=0);

        // base cutout 8.6
        Tz(total_H/2 - shaft_H -1.6 - body_H /2 -1.51) make_rounded_box_shafts(size=[47,47,body_H - 3], d=17.2, inset=-7);
       }
    }
  }
}
...
module stepper_nema_23_shafts(h=20) {
  make_rounded_box_shafts(size=[47,47,h + 3], d=4.9, shaft_d=0);
}
...
nema23_stepper_sprocket_20T_total_H = 19.05;
nema23_stepper_sprocket_20T_D = 12.3;

module pulley_gt2_10mm_sprocket_20() {
  silver()
  D() {
    U(){
      
      offset_Z = nema23_stepper_sprocket_20T_total_H/2;

      // bump 12.9 0.4
      ccylinder(d = 12.9, h=nema23_stepper_sprocket_20T_total_H);

      // hat 20 1
      Tz(offset_Z - 0.4 - 0.5) ccylinder(d = nema23_stepper_sprocket_20T_D + 4, h = 1);

      // gear 15.85 11.1
      Tz(offset_Z - 0.4 - 1 - 5.55) ccylinder(d = nema23_stepper_sprocket_20T_D, h = 12.1);

      // plate 20 1
      Tz(offset_Z - 1.4 - 11.1 - 0.5) ccylinder(d = nema23_stepper_sprocket_20T_D + 4, h = 1);

      // lower_shaft 12.9 9.7

    }
    U(){
      ccylinder(d = 8, h = nema23_stepper_sprocket_20T_total_H +1);
    }
  }
}
