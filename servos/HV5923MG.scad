...
// function get_5932_() = ;
// function get_5932_() = ;
function get_5932_bucketHeight() = bucketHeight;
function get_5932_bucketTopRetainerOffsetY() = bucketTopRetainerOffsetY;
function get_5932_bucketTopRetainerHeight() = bucketTopRetainerHeight;
...
module 5932_servo_bucket_top(withBolts=true, withServoCutout=true) {
  difference() {
    5932_servo_bucket(withBolts=withBolts, withServoCutout=withServoCutout, keyOversize=1.25);
    translate([0,0,-49.5]) {
...
module 5932_servo_bucket_bottom(withBolts=true, withServoCutout=true, left=true) {
  difference() {
    5932_servo_bucket(withBolts=withBolts, withServoCutout=withServoCutout, keyOversize=1.25, left=left);
    translate([0,0,49.5]) {
      cube([200,200,100], center = true);
    }
  }
...
module 5932_servo_bucket_frame_join() {
  difference() {
    translate([0,28,0]) {
      makeRoundedRhombus(bucket_flat_mount_X+3, bucket_flat_mount_Y-1, bucket_flat_mount_D, bucket_flat_mount_O, 15);
    }
    union() {
      5932_servo_bucket(withBolts=false, withServoCutout=false);
      5932_servo_bolts();
    }
  }
}

module 5932_servo_bucket(withBolts=true, withServoCutout=true, keyOversize=0, left=true) {
  retainerH = 42;
  difference() {
    union() {
      ...
      5932_servo_bucket_retainer(left=left);
...
module 5932_cable_support_bottom(left=true) {
  /* the base loop */ 
  support_inner_D = 13;
...
/* the side-ish loops */
module 5932_cable_support_side(left=true) {
  support_inner_D = 9;
  support_inner_offsetX = 2;
  support_outer_D = 15;
  support_H = 12;
...
module 5932_servo_bucket_retainer(left = true) {
  translate([0,bucketTopRetainerOffsetY - 6,-1.5]) {
    5932_cable_support_bottom(left = left);
  }
}

module 5932_servo_bolts() {
